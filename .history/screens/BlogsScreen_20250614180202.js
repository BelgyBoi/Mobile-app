...
const BlogsScreen = ({ navigation }) => {
  ...
  return (
   <ViewPort scroll contentContainerStyle={styles.scrollContent}>
      ...
      {filteredBlogPosts.length > 0 ? (
        filteredBlogPosts.map((post) => (
          <BlogCard
            ...
            date={post.date}
            onPress={() => navigation.navigate('BlogDetail', { blogPost: post })}
          />
        ))
      ) : (
        <View style={styles.noResultsContainer}>
          <BaselineText style={styles.noResultsText}>
            {blogPosts.length === 0 && !searchQuery
              ? 'Loading blog posts or none available.'
